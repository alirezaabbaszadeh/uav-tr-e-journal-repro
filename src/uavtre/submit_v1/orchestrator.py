...
from .manuscript_writer import write_sections, write_submission_text_artifacts
...
def _discover_resume_run(root: Path, campaign_id: str) -> str | None:
    runs_root = root / "outputs" / "pipeline_v1_runs"
...
def _write_exec_manifest(
    *,
    ...
    artifact_paths: list[Path] = [
        ctx.out_manuscript_dir / "main.pdf",
        ...
        ctx.out_submit_dir / f"TR_E_UPLOAD_PACK_{ctx.campaign_id}.zip",
...
def _read_git_sha(root: Path) -> str:
    ...
    if proc.returncode != 0:
        return "nogit-submit-v1"
    return (proc.stdout or "").strip() or "nogit-submit-v1"
...
def run_pipeline(
    *,
    ...
    if not selected_run_id:
        selected_run_id = f"submit_v1_{campaign_id}_{_ts()}"
    ...
    ctx = PipelineContext(
        ...
        logs_dir=logs_dir,
        out_submit_dir=root / "output_submit_v1" / "submission",
        out_manuscript_dir=root / "output_submit_v1" / "manuscript",
    )
    ...
    manifest_path = ctx.out_submit_dir / f"MANUSCRIPT_EXEC_MANIFEST_{campaign_id}.json"

    manuscript_root = root / "manuscript_submit_v1" / "tr_e"
    ...
    def s05() -> list[str]:
        ...
        if not res.passed:
            raise RuntimeError(
                f"claim guard failed: unresolved={res.unresolved} violations={res.policy_violations}"
            )
    ...
    def s08() -> list[str]:
        pdf = compile_manuscript(root=root, manuscript_root=manuscript_root, outdir=ctx.out_manuscript_dir)
        return [pdf.as_posix()]
    ...
    def s11() -> list[str]:
        # write a provisional manifest required by pack contract
        _write_exec_manifest(ctx=ctx, state=state, audit_path=audit_path, final_status="pre_pack")
    ...
    def s12() -> list[str]:
        check = check_pack(ctx.out_submit_dir / f"TR_E_UPLOAD_PACK_{campaign_id}.zip", campaign_id)
        anon_manifest_path = root / "submission_submit_v1" / "anonymous" / "BUNDLE_MANIFEST.json"
        cam_manifest_path = root / "submission_submit_v1" / "camera_ready" / "BUNDLE_MANIFEST.json"
        ...
        report = {
            ...
            "campaign_id": campaign_id,
            "checks": {
                "claim_guard_passed": json.loads(claim_report.read_text(encoding="utf-8")).get("passed", False),
                "audit_recheck_exists": audit_path.exists(),
                "manuscript_pdf_exists": (ctx.out_manuscript_dir / "main.pdf").exists(),
    ...
    steps: list[tuple[str, str, Callable[[], list[str]]]] = [
        ...
        ("S07", "Write manuscript sections", s07),
        ("S08", "Compile manuscript PDF", s08),
