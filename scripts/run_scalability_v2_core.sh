...
PROFILE_OVERRIDE="configs/profiles/scalability_v2_core.json"
...
if [[ -n "${OUTPUT_DIR:-}" ]]; then
  ...
  OUTPUT_DIR="outputs/scalability_v2_core"
else
  OUTPUT_DIR="outputs/scalability_v2_core_${TW_FAMILY}"
fi

if [[ -n "${BENCHMARK_DIR:-}" ]]; then
  BENCHMARK_DIR="${BENCHMARK_DIR}"
elif [[ "$TW_FAMILY" == "A" ]]; then
  BENCHMARK_DIR="benchmarks/frozen/scalability_v2_core"
else
  BENCHMARK_DIR="benchmarks/frozen/scalability_v2_core_${TW_FAMILY}"
fi

MERGED_OVERRIDE="configs/overrides/scalability_v2_core_${TW_FAMILY}_calibrated.json"
