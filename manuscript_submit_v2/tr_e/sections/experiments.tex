...
All numerical evidence in this manuscript is locked to an audited campaign with frozen benchmark instances and recorded execution metadata. Experiments are executed under WSL2 on a CPU-sharded setup (12 logical shards), because the core solvers used here (OR-Tools, HiGHS, PyVRP) are CPU-bound and do not benefit materially from consumer GPU acceleration.
...
The full run plan (including shard commands, configuration overrides, and frozen instance identifiers) is recorded in the campaign manifests and command log.
...
\item \textbf{PyVRP (hard TW)} as a baseline representing strict feasibility.
\end{itemize}
...
\item \textbf{Operational cost proxies:} energy proxy and communication risk summaries (mean arc outage risk and max-route risk).
