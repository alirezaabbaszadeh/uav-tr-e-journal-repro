...
| C2 | Soft-TW stress (Family B) reduces service quality for OR-Tools at medium/large sizes. | `outputs/paper_v2_core/table_main_kpi_summary.csv` vs `outputs/paper_v2_core_B/table_main_kpi_summary.csv`: OR-Tools `N=20` on-time `63.75% -> 28.75%`, tardiness `24.36 -> 53.64` min. | Supported |
...
| C5 | Statistical power is still limited for strong inferential claims in journal submission. | `outputs/main_table_v2_core/results_significance.csv` (significant rows=0), `outputs/main_table_v2_core_B/results_significance.csv` (significant rows=1); only one adjusted-significant result in Family B runtime comparison (`p_adj=0.0146`). | Caution |
